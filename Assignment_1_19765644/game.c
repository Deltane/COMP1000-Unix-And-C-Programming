...
void moveSnake(snake *pSnake, Map map_ptr, int rows, int cols)
{
    ...
    } else 
    {
        ...
        /* move neck to head, then update head position */
...
{
    int i;
    /* From tail to before the head, game over if head goes into body */
    for (i = 0; i < pSnake->size - 1; i++)
    {
        if ((pSnake->body[i].x == pSnake->headX) && (pSnake->body[i].y == pSnake->headY))
        {
            return 1;
        }
    }
